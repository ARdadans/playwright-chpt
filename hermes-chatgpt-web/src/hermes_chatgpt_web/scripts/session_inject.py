...
from chatgpt.browser import ChatGPTBrowser  # noqa: E402
from core.config import BASE_DIR as BASE  # noqa: E402
...
def main():
    ...
    try:
        info["localStorage_keys"] = page.evaluate("JSON.stringify(Object.keys(localStorage))")
    ...
    print(
        json.dumps(
            {k: (v if k != "body_head" else (v[:120] + "…" if isinstance(v, str) else v)) for k, v in info.items()},
            indent=2,
        )
    )
